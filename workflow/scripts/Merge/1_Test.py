...
subset_ref_9th = buildings_9th[buildings_9th['scenarios'] == 'reference']
subset_ref_CDA_9th = subset_ref_9th[subset_ref_9th['economy'] == '03_CDA']
subset_ref_8th = eighth_results[eighth_results['SCENARIO'] == 'Reference']
subset_ref_CDA_8th = subset_ref_8th[subset_ref_8th['REGION'] == '03_CDA']
#subset_ref_CDA_8th 
#subset_ref_CDA_9th
...
#merged_df = testing_copy_9th_trunc.merge(testing_copy_8th.loc[:, 2070], on=['sub1sectors', 'sub2sectors', 'fuels', 'subfuels'], how='left')
#merged_df = testing_copy_9th.merge(testing_copy_8th.loc[:, 2070], on=['sub1sectors', 'sub2sectors', 'fuels', 'subfuels'], how='left')
#merged_df = testing_copy_9th.merge(testing_copy_8th.loc[:, '2022':'2036'], on=['SCENARIO', 'REGION', 'FUEL', 'UNITS', 'NOTES'], how='left')

#merged_df = df1.merge(df2, on='key', how='left')
#in this example, df1 is maintained
#merge 9th maintained based on columns added to 8th test, only columns 2022 to 2050

#add a column with the 8th identifier to the 9th based on the dictionary
#merge the values from the 8th into the 9th based on the new column?
